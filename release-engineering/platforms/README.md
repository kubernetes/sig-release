...
- Official binary releases, container images, and deb/rpm packages are provided.
  Artifact builds are integrated into the release process.
...
rationale for why Tier 2 is the appropriate long-term tier. Platforms must have
CI in place before or alongside being added to the official build. Adding a
platform to the release builds without any CI is not permitted.
...
3. SIG Release evaluates the request and decides by lazy consensus.
4. On approval, the platform table below is updated, the change is announced on
...
1. SIG Release opens a GitHub issue describing which criteria are no longer met.
